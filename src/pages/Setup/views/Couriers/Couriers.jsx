...
import { getCouriers, setCourierUpdate } from '@/pages/Setup/api/CouriersApi'
...
const Couriers = () => {
  ...
  const changeStatus = (active) => {
    setCourierUpdate({ ID: product.ID, Inactive: !active }).then((res) => {
      if (!res.data.Error) {
  ...
  const getData = async (options) => {
    const resp = await getCouriers({...options, })
    return resp.data
  }
  ...
  return (
    <div className={style.couriersContainer}>
      <header className={style.headerContainer}>
        <h2 className={style.couriersTitle}>Couriers</h2>
